...
# input$counts gives .summary files; derive the actual count .txt paths
summary_files <- snakemake@input$counts
files <- gsub("\\.featureCounts\\.txt\\.summary$",
              ".featureCounts.txt", summary_files)
y <- readDGE(files, columns = c(1, 7), comment.char = "#")
count_matrix <- y$counts
# column names are full BAM paths; strip to sample ID
colnames(count_matrix) <- gsub(".*/(SRR[0-9]+)\\.sorted\\.bam", "\\1",
